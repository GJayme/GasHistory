...
class MainActivity : AppCompatActivity(), OnGasClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        gasActivityResultLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { resultado ->
        ...
        editGasActivityResultLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { resultado ->
            if (resultado.resultCode == RESULT_OK) {
            ...
            R.id.removeGasMi -> {
                with(AlertDialog.Builder(this)) {
                    ...
                    setPositiveButton("Sim") { _, _ ->
                        ...
                        gasAdapter.notifyDataSetChanged()
                        Snackbar.make(activityMainBinding.root, "Gas has removed", Snackbar.LENGTH_SHORT).show()
                    }
                    setNegativeButton("Não") { _, _ ->
                        Snackbar.make(activityMainBinding.root, "Canceling remove", Snackbar.LENGTH_SHORT).show()
                    }
                    create()
                }.show()
                true
